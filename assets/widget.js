...
$j(document).ready(function () {
    var domain = "qaq.scalacube.pro";
    var id = "#playvds-widget-qaq-scalacube-pro-widget";
        ...
        send(name + "/info", function (info) {
            if (info.type == "static") {
                send(name, function (content) {
                    ...
                    $j("#playvds-widget-qaq-scalacube-pro-widget").append("<link rel=\"stylesheet\" href=\"https://maxcdn.bootstrapcdn.com/font-awesome/4.5.0/css/font-awesome.min.css\">");

                    if ($j("#playvds-widget-qaq-scalacube-pro-widget #playvds-version-site").last().hasClass("online")) {
                        var str = $j("#playvds-widget-qaq-scalacube-pro-widget .playvds-page  p").last().text();
                        ...
                        $j("#playvds-widget-qaq-scalacube-pro-widget .playvds-page p").last().remove();
                        setTable('green', "server online|" + mas[0] + "/" + mas[1], info.title, contn);
                        $j("#playvds-widget-qaq-scalacube-pro-widget .playvds-status").last().addClass("fa fa-check-circle green");


                    } else {
                        setTable('red', "server Offline", info.title, "");
                        $j("#playvds-widget-qaq-scalacube-pro-widget .playvds-status").addClass("fa fa-times-circle red")
                    }
                });
            }
        }, function (error) {
    ...
    function setTable(colorfont, text, title, contn) {
        $j("#playvds-widget-qaq-scalacube-pro-widget .playvds-page p").last()
